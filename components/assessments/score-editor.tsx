...
import {saveScores,publishScores,correctReturnedScores} from "@/features/assessments/actions";
import {SubmitButton} from "@/components/ui/submit-button";
import {ActionFeedback} from "@/components/ui/feedback";
import type {Database} from "@/lib/supabase/database.types";
import type {RosterStudent} from "@/features/classes/types";
type Assessment=Database["public"]["Tables"]["assessments"]["Row"];
export function ScoreEditor({assessment:a,students,scores,canManage,canCorrect=false}:{assessment:Assessment;students:RosterStudent[];scores:{student_id:string;score:number}[];canManage:boolean;canCorrect?:boolean}){
 const [save,saveAction,saving]=useActionState(canCorrect?correctReturnedScores:saveScores,{}),[publish,publishAction,publishing]=useActionState(publishScores,{});
 ...
 const published=Boolean(a.published_at),tooLarge=students.length>500||scores.length>500,editable=canManage&&(!published||canCorrect)&&!tooLarge;
 const values=new Map(scores.map(s=>[s.student_id,s.score]));
 return <section className="border border-border rounded-lg bg-white p-5 mt-5"><div className="flex flex-wrap justify-between gap-3"><div><h2 className="text-xl font-semibold text-navy">{a.title}</h2><p className="text-sm text-muted mt-2">{a.assessment_date} · Out of {a.max_score}</p></div><span className="text-sm font-medium text-primary">{published?(canCorrect?"Published · Correction enabled":"Published · Read-only"):"Draft · Not visible to students"}</span></div>
 ...
 {canCorrect&&<p className="text-sm text-amber-900 mb-4">The adviser returned this period. Corrections require a reason. Changes to published scores are immediately visible in student results; submitted grade snapshots remain unchanged until resubmission.</p>}
