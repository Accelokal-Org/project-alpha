...
export function InvitationForm({schoolId,teachers,students}:{schoolId:string;teachers:{id:string;display_name:string;user_id:string|null}[];students:{id:string;display_name:string;user_id:string|null}[]}) {
 ...
 <ActionFeedback pending={pending} error={state.error} success={state.success}/><SubmitButton pendingLabel="Sending invitation…" disabled={pending||!roles.length||(needsProfile&&!profiles.length)}>{pending?"Sending invitation…":"Create account & send invite"}</SubmitButton>
 </form></section>;
}
