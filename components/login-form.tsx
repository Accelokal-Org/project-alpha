...
export function LoginForm({ configured, admin = false, destination }: { configured: boolean; admin?: boolean; destination?: string | null }) {
 ...
 return <form aria-busy={pending} action={action} className="space-y-5">
  ...
  <ActionFeedback pending={pending} error={state.error}/><SubmitButton pendingLabel="Signing in…" type="submit" className="w-full" disabled={pending || !configured}>{pending ? "Signing in…" : "Sign in"}</SubmitButton>
 </form>;
}
