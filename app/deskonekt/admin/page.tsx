...
export default async function AdminPage({searchParams}:{searchParams:Promise<{school?:string;tab?:string}>}) {
 ...
 return <div className="min-h-screen"><a href="#admin-main" className="sr-only focus:not-sr-only">Skip to administration</a><header className="bg-white border-b border-border px-5 sm:px-8 py-3 flex justify-between items-center gap-4"><Link href="/deskonekt/admin" className="flex gap-3 items-center"><Image src="/brand/deskonekt-icon-light.png" width={404} height={394} alt="" className="w-10 h-auto" sizes="40px" /><div><span className="text-lg font-semibold text-navy">{brand.name}</span><p className="text-[11px] text-muted">Superadmin · {brand.byline}</p></div></Link><div className="flex items-center gap-4"><Link href="/teacher" className="text-sm text-primary">Open teacher workspace</Link><span className="hidden sm:block text-xs text-muted">{d.user.email}</span><form action={logout}><Button variant="outline" size="sm">Sign out</Button></form></div></header>
 <main id="admin-main" className="max-w-7xl mx-auto p-5 sm:p-8">
  <div className="flex flex-wrap justify-between gap-4 items-end mb-6"><div><p className="text-xs text-muted mb-2">Platform administration</p><h1 className="text-2xl font-semibold text-navy">{d.selected?.name??"Schools"}</h1><p className="text-sm text-muted mt-2">{d.selected?"Configure the school and connect accounts.":"Manage schools and academic records."}</p></div><div className="flex gap-4 text-xs text-muted"><span>Invite emails · Planned</span><span>Billing · Planned</span></div></div>
  {d.selected ? <>
   <form method="get" className="flex flex-wrap gap-3 items-end mb-5"><div className="flex-1 max-w-md"><label htmlFor="school">School</label><select id="school" name="school" defaultValue={d.selected.id} className="w-full">{d.schools.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></div><Button variant="outline" type="submit">Open school</Button><Link className="text-sm text-primary py-2" href="/deskonekt/admin">All schools / add school</Link></form>
  ...
  <p className="text-xs text-muted mt-7">Available now: school setup, role connections, assignments, and rosters. Assessment, attendance, grade workflows, invitations, and billing will be added in later milestones.</p>
 </main></div>;
}
