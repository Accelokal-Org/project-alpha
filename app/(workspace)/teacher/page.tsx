...
export default async function Teacher({ searchParams }: { searchParams: Promise<{ view?: string; school?: string; year?:string; page?:string; status?:string; batch?:string }> }) {
 ...
 if(params.view==="accounts") return <AppShell adminAccess={isAppManager(memberships)} active="workspace"><SchoolAccounts school={params.school} batch={params.batch}/></AppShell>;
 ...
 return <AppShell adminAccess={isAppManager(memberships)} active={params.view === "classes" ? "classes" : "workspace"}>{(isAppManager(memberships)||memberships.some(m=>m.role==="SCHOOL_HEAD"))&&<div className="flex gap-5"><Link href="/teacher?view=accounts" className="text-sm text-primary inline-block mb-5">School accounts</Link><Link href="/teacher?view=grading" className="text-sm text-primary inline-block mb-5">School grading setup</Link></div>}<Workspace assignments={assignments} classesView={params.view === "classes"} />{params.view!=="classes"&&<UpcomingLessons assignments={assignments}/>}</AppShell>;
}
