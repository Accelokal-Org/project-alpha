...
export default async function Teacher({ searchParams }: { searchParams: Promise<{ view?: string; school?: string }> }) {
 const { memberships } = await requireSession();
 const [assignments, params] = await Promise.all([getAssignments(),searchParams]);
 if(params.view==="grading") return <AppShell adminAccess={isAppManager(memberships)} active="workspace"><SchoolGradingWorkspace school={params.school}/></AppShell>;
