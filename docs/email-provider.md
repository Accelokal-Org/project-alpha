...
4. Set Supabase Authentication URL Configuration → Site URL to the actual deployed Deskonekt URL. Redirect URLs must match implemented application flows; the invitation route is `/auth/accept`; follow `docs/account-invitations.md`. Password recovery remains unimplemented.
...
SMTP configuration enables delivery for emails requested by Supabase Auth. Deskonekt now supports [superadmin invitations and password setup](account-invitations.md). Ordinary password sign-in does not send an email. The bootstrap script still sends no email. Other application notifications and password recovery remain future workflows.

## Setup status

The provider settings above were verified against official documentation on 2026-09-14. The user owns deskonekt.com, manages DNS through Vercel, and reports provider setup complete. This workspace has not independently verified DNS or SMTP configuration. No emails have been sent and no external settings have been changed by this workspace.
